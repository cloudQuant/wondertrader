#include "WtCtaStraStatisticalArbitrageFact.h"
...
const char* FACT_NAME = "WtCtaStraStatisticalArbitrageFact";
...
CtaStrategy* WtStraFact::createStrategy(const char* name, const char* id)
{
	...
	if (strcmp(name, "StatisticalArbitrage") == 0) {
		std::cout << "StatisticalArbitrage init success" << std::endl;
...
{
	cb(FACT_NAME, "StatisticalArbitrage", false);
