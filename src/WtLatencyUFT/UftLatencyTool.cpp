/*!
 * @file UftLatencyTool.cpp
 * @project	WonderTrader
 *
 * @author Wesley
 * @date 2020/03/30
 * 
 * @brief 超高频交易延迟测试工具实现文件
 * @details 本文件实现了UFT(超高频交易)延迟测试的全流程，包括测试解析器、交易器和策略的实现，以及延迟测试工具类的实现
 */
...
/**
 * @brief 测试UFT模块延迟性能
 * @details 创建并初始化UftLatencyTool实例，然后运行延迟测试流程
 */
void test_uft()
{
	// 创建延迟测试工具实例
...
namespace uft
{
	...
	/**
	 * @brief 测试用交易器类
	 * @details 模拟的交易器实现，用于测试超高频交易时的交易处理流程
	 */
	...
	/**
	 * @brief 测试用超高频策略类
	 * @details 模拟的超高频策略实现，用于测试超高频交易引擎的延迟性能
	 */
	...
	public:
		 ...
		 * @details 当收到新的Tick数据时调用，执行多头建仓操作，用于测试超高频交易引擎的冲击
		 */
		virtual void on_tick(IUftStraCtx* ctx, const char* code, WTSTickData* newTick)
		{
			//WTSLogger::debug("{}", __FUNCTION__);
			// 多头建仓，价格2300，数量1手，测试超高频交易引擎的响应速度
	...
	/**
	 * @brief UftLatencyTool类构造函数
	 * @details 创建超高频交易延迟测试工具实例
	 */
	UftLatencyTool::UftLatencyTool()
	{
		// 因为成员变量都是对象，会自动调用默认构造函数，这里不需要特殊的初始化
	}


	/**
	 * @brief UftLatencyTool类析构函数
	 * @details 清理超高频交易延迟测试工具资源
	 */
	UftLatencyTool::~UftLatencyTool()
	{
		// 成员变量会自动析构，不需要特殊的清理代码
	}

	/**
	 * @brief 初始化延迟测试工具
	 * @return 初始化是否成功
	 * @details 加载日志配置、主配置文件和基础数据文件，设置测试参数，并初始化引擎、模块和策略
	 */
	 ...
	 * @brief 初始化测试策略
	 * @return 初始化是否成功
	 * @details 创建策略上下文对象，设置策略实例，关联交易器并添加到引擎中
	 */
	 ...
	 * @param cfg 引擎配置参数
	 * @return 初始化是否成功
	 * @details 设置并初始化UFT引擎，配置基础数据管理器和交易适配器管理器
	 */
	 ...
	 * @brief 初始化测试模块
	 * @return 初始化是否成功
	 * @details 创建并初始化测试用的行情解析器和交易器，并将其添加到相应的适配器管理器中
	 */
	...
	/**
	 * @brief 执行延迟测试
	 * @details 绑定CPU核心，启动解析器、交易器和引擎，然后运行测试解析器生成模拟的tick数据
	 */
	void UftLatencyTool::run()
	{
		// 如果指定了CPU核心，则绑定当前线程到指定核心
		if (_core != 0)
		{
			// 注意核心编号从0开始，所以需要-1
